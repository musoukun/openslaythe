## Changes the charges for a given artifact(s). Can target either an artifact type or a specific
## instance of an artifact.
## NOTE: Unlike ActionIncreaseArtifactCharge, this will NOT proc any artifact charge actions.
extends BaseAction
...
func perform_action():
	...
	for action_interceptor_processor in action_interceptor_processors:
		...
		var artifact_charges: int = action_interceptor_processor.get_shadowed_action_values("artifact_charges", 1)
		...
		if artifact_id != "":
			...
			for artifact_data: ArtifactData in artifacts_with_artifact_id:
				artifact_data.set_artifact_counter(artifact_charges)
